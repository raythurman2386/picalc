...
# Install picalc from a prebuilt GitHub Release tarball.
#
...
RELEASE_BASE_URL="${PICALC_RELEASE_BASE_URL:-$DEFAULT_RELEASE_BASE_URL}"
VERSION=""
...
Environment:
  PICALC_RELEASE_BASE_URL  Override the release artifact base URL

Examples:
  $0                # latest release into ~/.local
  $0 0.1.2          # a specific release
  $0 --prefix /opt/picalc
EOF
}
...
main() {
    ...
    local tarball="picalc-${version_no_v}-${triple}.tar.gz"

    echo "==> Platform: $triple"
    echo "==> Version:  $version_tag"
    echo "==> Prefix:   $PREFIX"

    if [[ -x "$PREFIX/bin/picalc" ]] && [[ "$FORCE" != true ]]; then
        echo "==> picalc is already installed at $PREFIX/bin/picalc"
    ...
    # checksums (and therefore the tarball) were produced by the picalc
    ...
    local pubkey_file
    pubkey_file="$tmp_dir/picalc-signing-key.pub"
    ...
    local package_dir
    package_dir="$(find "$extract_dir" -maxdepth 1 -type d -name 'picalc-'"$version_no_v"'-*' | head -1)"
    ...
    echo "==> Installed picalc $version_tag into $PREFIX"
}
