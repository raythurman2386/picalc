...
# mode (PICALC_RELEASE_BASE_URL pointing at a filesystem path). Verifies that:
...
TARBALL="picalc-${VERSION}-${TRIPLE}.tar.gz"
...
make_fake_tarball() {
    local stage="$WORK/stage/picalc-${VERSION}-${TRIPLE}"
    rm -rf "$WORK/stage"
    mkdir -p "$stage"
...
mkdir -p "$PREFIX/bin"
printf '#!/usr/bin/env bash\necho "picalc %s (fake)"\n' "$(basename "$0")" > "$PREFIX/bin/picalc"
chmod +x "$PREFIX/bin/picalc"
EOF
    chmod +x "$stage/install.sh"
    tar -czf "$RELEASE_DIR/$TARBALL" -C "$WORK/stage" "picalc-${VERSION}-${TRIPLE}"
}
...
run_install() {
    ...
    PICALC_RELEASE_BASE_URL="$WORK/release" \
...
[[ -x "$INSTALL_DIR/bin/picalc" ]] || fail "picalc binary not installed"
...
[[ ! -e "$INSTALL_DIR/bin/picalc" ]] || fail "tampered tarball must not be installed"
...
[[ ! -e "$INSTALL_DIR/bin/picalc" ]] || fail "tarball must not be installed on bad signature"
...
[[ ! -e "$INSTALL_DIR/bin/picalc" ]] || fail "tarball must not be installed without a signature"
