...
# Generate an Ed25519 signing keypair for picalc release signing.
#
...
OUT_DIR="${1:-$HOME/.picalc/signing}"

usage() {
    cat <<EOF
Usage: $0 [OUT_DIR]

Generate an Ed25519 keypair for signing picalc releases.

  OUT_DIR  Directory to write the keys (default: \$HOME/.picalc/signing)
...
in the netinstaller (or the picalc-signing-key.pub file).
EOF
}
